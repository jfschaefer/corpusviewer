...
abstract class AbstractPreviewScalingFunction {
  ...
  def reciprocal(x: Double): Double = 1 / function(x)

  def reciprocalIntegral(x: Double): Double

  def reciprocalIntegralFromMinusOne(x: Double): Double = {
    reciprocalIntegral(x) - reciprocalIntegral(-1d)
  }
  ...
  def normalizedIntegralInverse(x: Double): Double = {
    ...
    secantMethod(-2, 2, normalizedIntegral(_) - x, 25)
  }
...
class PolynomialScalingFunction extends AbstractPreviewScalingFunction {
  override def function(x: Double): Double = if (x < -1 || x > 1) 0.25 else 1 - (x*x) * (1.5 - 0.75 * x*x)
